...
dv new --plugin-local ~/work/second-brain # create an agent with a local plugin bind-mounted
...
`dv new --plugin-local PATH` bind-mounts a plugin you already have checked out on the host into the new agent's `plugins/<name>` instead of cloning it, so the host directory is the single source of truth — edit it on the host and the container sees the changes (and vice versa). The plugin name is read from `plugin.rb`'s `# name:` line, falling back to the directory name. When you don't pass an agent name, the agent is named after the plugin (e.g. `dv new --plugin-local ~/work/second-brain` creates an agent named `second-brain`) rather than an autogenerated name; pass an explicit name to override. `--plugin-local` is repeatable and can be combined with `--plugin` (clone) in the same command; when several plugins are mounted the agent is named after the first, so pass an explicit name if you want a different one. Two plugins that resolve to the same `plugins/<name>` are rejected up front.
